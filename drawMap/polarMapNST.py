...
def export_Fig2a(in_slopetif, out_folder):
    ...
    cs1 = ax1.imshow(export_slope,
                        ...
                        zorder=1)
    # 在子图2中绘制南极地区的栅格数据
    cs2 = ax2.imshow(export_slope,
                         transform=ccrs.PlateCarree(), cmap=cmap,
                         vmax=1, vmin=-1,
                         zorder=1)

    # 在子图1中绘制图例
    cbar_ax = ax1.inset_axes([0.9, 0.02, 0.5, 0.03])
    ...
    # # 在子图2中绘制图例
    # cbar_ax2 = ax2.inset_axes([0.4, 0.02, 0.4, 0.03])
    ...
    # cb2.set_label(r'Slope (10$^2$ yr$^{-1}$)', labelpad=-29, x=0.5)
    # cb2.ax.xaxis.set_minor_locator(tk.MultipleLocator(0.25))

    plt.rcParams['pdf.fonttype'] = 42
    fig.savefig(os.path.join(out_folder, 'export_figNST102.pdf'), format='pdf', dpi=300, bbox_inches='tight',
                pad_inches=0.1)
